...
Wiele rzeczy jest w trakcie fazy rozwoju, a elementy PvE takie jak czynniki środowiskowe oraz zajazdy wrogich frakcji są planowane. Dodatkowo, trwają prace nad naszą własną implementacją podróży kosmicznych.
...
Na lipiec 2026, następujące systemy oraz mechaniki zostały dodane, bądź zmienione z stanu domyślnego:
...
Część modów (na przykład Applied Energistics 2, Stargate, TARDIS) są wymienione w modpacku, lecz ich zawartość nie jest dostępna, lecz ulegnie to zmianie.
W szczególności AE2 będzie dostępne po możliwości lotów kosmicznych.
...
- Tian-mi • Chińskie tłumaczenie i inne,
...
- Dodaj moda: `packwiz cf add <project name>` ([Full Usage](https://packwiz.infra.link/tutorials/creating/adding-mods/))
- Odświerz Hash: `packwiz refresh`
- Wypisz listę modów: `packwiz list`
- Aktualizacja wersji paczki: przepisz numer wersji z [pack.toml](pack.toml)
- Aktualizacja wersji: `packwiz update <mod>/--all`
- Mod tylko po stronie klienta: [rewrite <mod>.pw.toml#side manually](https://packwiz.infra.link/reference/pack-format/mod-toml/#properties)
