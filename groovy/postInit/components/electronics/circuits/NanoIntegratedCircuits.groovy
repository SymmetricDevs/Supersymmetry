...
Deposition.generateChemicalVaporDepositionRecipe('wafer.cmos.step_one', 'wafer.cmos.step_two', 400, ['silane' : 50, 'ammonia' : 50], ['hydrogen' : 150]) // CMP stop layer for STI formation
...
def generateCMOSFabrication(String componentName, int circ) {
    ...
    Lithography.generatePhotolithographyRecipes('wafer.cmos.step_two', 'wafer.' + componentName + '.step_three', 'acrylate_resist', 'mask_set.' + componentName, true) // Define STI pattern
    Etching.generateReactiveIonEtchingRecipe('wafer.' + componentName + '.step_three', 'wafer.' + componentName + '.step_four', 'silicon_nitride.silane', 400) // Etch into silicon to form trenches
    ...
    Etching.generateWetEtchingRecipe('wafer.' + componentName + '.step_nine', 'wafer.' + componentName + '.step_ten', 'silicon_nitride.silane', 400, false) // Etch away silicon nitride CMP stop layer

    // NMOS well formation
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_ten', 'wafer.' + componentName + '.step_eleven', 'acrylate_resist', 'mask_set.' + componentName, true)
    ...
    // PMOS well formation
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_ten', 'wafer.' + componentName + '.step_fifteen', 'acrylate_resist', 'mask_set.' + componentName, true)
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_twenty_one', 'wafer.' + componentName + '.step_twenty_two', 400, ['silane' : 50], ['hydrogen' : 100], EV) // Grow dummy gate
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_twenty_two', 'wafer.' + componentName + '.step_twenty_three', 400, ['silane' : 50, 'ammonia' : 50], ['hydrogen' : 150], EV) // Hard mask layer for dummy gate
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_twenty_three', 'wafer.' + componentName + '.step_twenty_four', 'acrylate_resist', 'mask_set.' + componentName, true) // Define gate pattern
    Etching.generateReactiveIonEtchingRecipe('wafer.' + componentName + '.step_twenty_four', 'wafer.' + componentName + '.step_twenty_five', 'silicon_nitride.silane', 400)
    ...
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_twenty_eight', 'wafer.' + componentName + '.step_twenty_nine', 'acrylate_resist', 'mask_set.' + componentName, true) // Define source/drain pattern for NMOS to form halo and extension implants
    ...
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_twenty_eight', 'wafer.' + componentName + '.step_thirty_three', 'acrylate_resist', 'mask_set.' + componentName, true) // Define source/drain pattern for PMOS to form halo and extension implants
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_thirty_six', 'wafer.' + componentName + '.step_thirty_seven', 400, ['silane' : 50, 'ammonia' : 50], ['hydrogen' : 150], EV) // NOTE: original line specified TEOS for silicon nitride but this doesn't make sense?
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_thirty_seven', 'wafer.' + componentName + '.step_thirty_eight', 'acrylate_resist', 'mask_set.' + componentName, true)
    ...
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_forty', 'wafer.' + componentName + '.step_forty_one', 'acrylate_resist', 'mask_set.' + componentName, true) // Protect NMOS while etching PMOS source/drain areas
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_forty_four', 'wafer.' + componentName + '.step_forty_five', 400, ["silane" : 50, "germanium_tetrahydride" : 50], ["hydrogen" : 200], EV) // Epitaxial deposition of SiGe in PMOS source/drain recesses
    ...
    Deposition.generateSputteringRecipes('wafer.' + componentName + '.step_forty_seven', 'wafer.' + componentName + '.step_forty_eight', ['nickel' : 480, 'platinum' : 20]) // Deposit nickel for silicidation, platinum is added to improve thermal stability of silicide
    ...
    Etching.generateWetEtchingRecipe('wafer.' + componentName + '.step_forty_nine', 'wafer.' + componentName + '.step_fifty', 'nickel_salicide', 400, false) // Etch away unreacted nickel: H2SO4, H2O2.
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_fifty_one', 'wafer.' + componentName + '.step_fifty_two', 400, ['silane' : 50, 'ammonia' : 50], ['hydrogen' : 150], EV) // T-CESL
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_fifty_two', 'wafer.' + componentName + '.step_fifty_three', 'acrylate_resist', 'mask_set.' + componentName, true) // Remove T-CESL from PMOS areas
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_fifty_five', 'wafer.' + componentName + '.step_fifty_six', 400, 'silicon_dioxide.silane') // Interlayer dielectric
    ...
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_sixty', 'wafer.' + componentName + '.step_sixty_one', 'acrylate_resist', 'mask_set.' + componentName, true) // Mask PMOS areas to implant NMOS workfunction metal
    ...
    Deposition.generateSputteringRecipes('wafer.' + componentName + '.step_sixty_four', 'wafer.' + componentName + '.step_sixty_five', 'aluminium', 400) // Deposit aluminium gate fill
    ...
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_sixty_eight', 'wafer.' + componentName + '.step_sixty_nine', 'acrylate_resist', 'mask_set.' + componentName, true) // Define plug pattern
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_seventy_three', 'wafer.' + componentName + '.step_seventy_four', 400, ['tungsten_hexafluoride' : 50, 'hydrogen' : 150], ['hydrogen_chloride', 300], EV) // Tungsten hexafluoride plug fill
    ...
    for (l in 1..9) {
        ...
        def resist = l <= 3 ? 'acrylate_resist' : (l <= 6 ? 'polyhydroxystyrene_resist' : 'novolac_resist')

        Deposition.generateChemicalVaporDepositionRecipe(input, 'wafer.' + componentName + '.beol_' + numberTab[l] + '.step_one', 400, ['silicon_hexafluoride' : 50, 'oxygen' : 50], ['fluorine' : 25], EV) // Deposit low-k dielectric
        ...
        Deposition.generateSputteringRecipes('wafer.' + componentName + '.beol_' + numberTab[l] + '.step_four', 'wafer.' + componentName + '.beol_' + numberTab[l] + '.step_five', ['tantalum' : 100, 'copper' : 100]) // Adhesion layer for copper, seed layer
    ...
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.beol_nine.step_seven', 'wafer.' + componentName + '.step_one_hundred_forty_eight', 400, ['silane' : 50, 'oxygen' : 75], ['steam' : 25], EV) // Passivation layer deposition
    Deposition.generateChemicalVaporDepositionRecipe('wafer.' + componentName + '.step_one_hundred_forty_eight', 'wafer.' + componentName + '.step_one_hundred_forty_nine', 400, ['silane' : 50, 'ammonia' : 50], ['hydrogen' : 150], EV)
    ...
    Lithography.generateResistStrippingRecipes('wafer.' + componentName + '.step_one_hundred_fifty', 'wafer.' + componentName + '.step_one_hundred_fifty_two', 1, false)
    Deposition.generateSputteringRecipes('wafer.' + componentName + '.step_one_hundred_fifty_two', 'wafer.' + componentName + '.step_one_hundred_fifty_three', ['chromium' : 200, 'copper' : 200]) // Deposit under-bump metallization layers for solder bump attachment
    Lithography.generatePhotolithographyRecipes('wafer.' + componentName + '.step_one_hundred_fifty_three', 'wafer.' + componentName + '.step_one_hundred_fifty_four', 'novolac_resist', 'mask_set.' + componentName, true) // Define solder bump pattern
