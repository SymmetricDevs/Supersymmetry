import static prePostInit.Recipemaps.*
import static gregtech.api.GTValues.*

// Tier 1
BR.recipeBuilder()
    .inputs(ore('dustLanthanumIiiCarbonate') * 14)
    .fluidInputs(fluid('hydrofluoric_acid') * 6000)
    .outputs(metaitem('dustLanthanumIiiFluoride') * 8)
    .fluidOutputs(fluid('carbon_dioxide') * 3000)
    .fluidOutputs(fluid('water') * 9000)
    .duration(80)
    .EUt(VA[LV])
    .buildAndRegister()

REACTION_FURNACE.recipeBuilder()
    .notConsumable(metaitem('crucible.tantalum'))
    .inputs(ore('dustLanthanumIiiFluoride') * 8)
    .inputs(ore('dustHighPurityCalcium') * 3)
    .fluidInputs(fluid('argon') * 50)
    .outputs(metaitem('dustLanthanum') * 2)
    .outputs(metaitem('dustFluorite') * 9)
    .duration(200)
    .EUt(960)
    .buildAndRegister()

// Tier 2
MIXER_SETTLER.recipeBuilder()
    .fluidInputs(fluid('distilled_water') * 1000)
    .fluidInputs(fluid('lanthanum_extract') * 10000)
    .fluidOutputs(fluid('lanthanum_nitrate_solution') * 1000)
    .fluidOutputs(fluid('aliquat_336_extraction_mixture') * 10000)
    .duration(80)
    .EUt(VA[MV])
    .requiredCells(2)
    .buildAndRegister()

DISTILLERY.recipeBuilder()
    .fluidInputs(fluid('lanthanum_nitrate_solution') * 1000)
    .outputs(metaitem('dustLanthanumNitrate') * 13)
    .fluidOutputs(fluid('water') * 1000)
    .duration(20)
    .EUt(VA[LV])
    .buildAndRegister()

BR.recipeBuilder()
    .inputs(ore('dustLanthanumNitrate') * 13)
    .fluidInputs(fluid('hydrofluoric_acid') * 3000)
    .outputs(metaitem('dustLanthanumIiiFluoride') * 4)
    .fluidOutputs(fluid('diluted_nitric_acid') * 6000)
    .duration(80)
    .EUt(VA[LV])
    .buildAndRegister()

// Lanthanothermy Recycling
BR.recipeBuilder()
    .inputs(ore('dustLanthanumOxide') * 5)
    .fluidInputs(fluid('hydrofluoric_acid') * 6000)
    .outputs(metaitem('dustLanthanumIiiFluoride') * 8)
    .fluidOutputs(fluid('water') * 9000)
    .duration(80)
    .EUt(60)
    .buildAndRegister()

// Lanthanum carbonate
BR.recipeBuilder()
    .inputs(ore('dustSodaAsh') * 18)
    .fluidInputs(fluid('lanthanum_nitrate_solution') * 1000)
    .fluidInputs(fluid('distilled_water') * 4000)
    .outputs(metaitem('dustLanthanumIiiCarbonate') * 14)
    .fluidOutputs(fluid('sodium_nitrate_solution') * 6000)
    .EUt(VA[LV])
    .duration(100)
    .buildAndRegister()

//Lanthanum nitrate (pre-IV)
ROASTER.recipeBuilder()
    .inputs(ore('dustLanthanum') * 2)
    .fluidInputs(fluid('oxygen') * 3000)
    .outputs(metaitem('dustLanthanumOxide') * 5)
    .duration(120)
    .EUt(VA[EV])
    .buildAndRegister()

BR.recipeBuilder()
    .inputs(ore('dustLanthanumOxide') * 5)
    .fluidInputs(fluid('nitric_acid') * 6000)
    .fluidOutputs(fluid('diluted_lanthanum_nitrate_solution') * 3000)
    .duration(80)
    .EUt(VA[MV])
    .buildAndRegister()

DISTILLERY.recipeBuilder()
    .fluidInputs(fluid('diluted_lanthanum_nitrate_solution') * 3000)
    .outputs(metaitem('dustLanthanumNitrate') * 13)
    .fluidOutputs(fluid('water') * 3000)
    .duration(80)
    .EUt(VA[MV])
    .buildAndRegister()

MIXER.recipeBuilder()
    .inputs(ore('dustLanthanumNitrate') * 13)
    .fluidInputs(fluid('water') * 1000)
    .fluidOutputs(fluid('lanthanum_nitrate_solution') * 1000)
    .duration(120)
    .EUt(VA[HV])
    .buildAndRegister()