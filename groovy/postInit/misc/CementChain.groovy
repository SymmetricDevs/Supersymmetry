...
for (fuel in sintering_fuels) {
    ...
    if (fuel.isPlasma) {
        ...
        .inputs(ore('dustLimestone') * 4)
        .fluidInputs(fluid(fuel.name) * (fuel.amountRequired * 4))
        .outputs(metaitem('hot.cement.clinker') * 4)
        .fluidOutputs(fluid(fuel.byproduct) * (fuel.byproductAmount * 4))
        ...
        .inputs(ore('dustClay') * 4)
        .inputs(ore('dustLimestone') * 4)
        .inputs(ore('dustGypsum'))
        .fluidInputs(fluid(fuel.name) * (fuel.amountRequired * 4))
        .outputs(metaitem('hot.cement.clinker') * 4)
        .chancedOutput(metaitem('hot.cement.clinker') * 4, 5000, 0)
        .fluidOutputs(fluid(fuel.byproduct) * (fuel.byproductAmount * 4))
        .duration(fuel.duration * 4)
        .EUt(Globals.voltAmps[3])
        .buildAndRegister()
        ...
        for (comburent in sintering_comburents) {
            ...
            .inputs(ore('dustLimestone') * 4)
            .fluidInputs(fluid(fuel.name) * (fuel.amountRequired * 4))
            .fluidInputs(fluid(comburent.name) * (comburent.amountRequired * 4))
            .outputs(metaitem('hot.cement.clinker') * 4)
            .fluidOutputs(fluid(fuel.byproduct) * (fuel.byproductAmount * 4))
            .duration(fuel.duration + comburent.duration * 4)
